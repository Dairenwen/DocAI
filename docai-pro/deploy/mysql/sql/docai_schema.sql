...
(
    id            BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '文件唯一标识',
    user_id       BIGINT       NOT NULL COMMENT '用户ID',
    file_name     VARCHAR(255) NOT NULL COMMENT '原始文件名',
    file_path     VARCHAR(500) COMMENT '文件存储路径',
    file_size     BIGINT COMMENT '文件大小(字节)',
    oss_key       VARCHAR(300) COMMENT 'OSS文件key',
    upload_status TINYINT   DEFAULT 1 COMMENT '上传状态：1-成功 0-失败 2-上传中 3-处理中 4-完成',
    INDEX idx_user_id (user_id),
    INDEX idx_upload_status (upload_status)
) COMMENT '文件表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
) COMMENT '文件与MySQL表映射' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    INDEX idx_file_id (file_id)
) COMMENT 'Excel表头与MySQL字段映射' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    status       TINYINT  DEFAULT 0 COMMENT '状态：0-处理中 1-成功 2-失败 3-部分成功',
    INDEX idx_user_id (user_id)
) COMMENT 'AI请求记录表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    file_type     VARCHAR(20)  NOT NULL COMMENT '文件类型：docx/md/txt/xlsx',
...
(
    ...
    field_name      VARCHAR(200) NOT NULL COMMENT '原始字段名',
    field_value     TEXT COMMENT '字段值',
    field_type      VARCHAR(20)  DEFAULT 'text' COMMENT '类型：text/date/number/phone/org/person/enum',
    aliases         JSON COMMENT '别名列表',
    source_text     TEXT COMMENT '原文证据',
    source_location VARCHAR(200) COMMENT '原文位置',
    confidence      DECIMAL(5,4) DEFAULT 0.0 COMMENT '置信度0-1',
    created_at      DATETIME     DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_doc_id (doc_id),
    INDEX idx_field_key (field_key),
    INDEX idx_field_type (field_type)
) COMMENT '抽取字段表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
) COMMENT '字段别名词典表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    updated_at      DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_user_id (user_id)
) COMMENT '模板文件表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    final_value      TEXT COMMENT '最终填写值',
    final_field_id   VARCHAR(100) COMMENT '最终字段ID',
    final_confidence DECIMAL(5,4) DEFAULT 0.0 COMMENT '最终置信度',
    decision_mode    VARCHAR(30)  DEFAULT 'rule_only' COMMENT '决策模式',
    reason           TEXT COMMENT '决策原因',
    INDEX idx_audit_id (audit_id),
    INDEX idx_template_id (template_id),
    INDEX idx_slot_id (slot_id)
) COMMENT '填写决策表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
(
    ...
    source_doc_name     VARCHAR(255) COMMENT '来源文档名',
    source_text         TEXT COMMENT '原文证据',
    reason              TEXT COMMENT '决策原因',
    candidates_summary  TEXT COMMENT '候选摘要',
    created_at          DATETIME    DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_audit_id (audit_id),
    INDEX idx_template_id (template_id)
...
(
    ...
    linked_doc_id   BIGINT       NULL COMMENT '关联的源文档ID',
    linked_doc_name VARCHAR(255) NULL COMMENT '关联文档名称',
    pinned          TINYINT      NOT NULL DEFAULT 0 COMMENT '是否置顶 0-否 1-是',
    created_at      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    INDEX idx_user_id (user_id),
    INDEX idx_updated_at (updated_at)
) COMMENT '对话会话表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
...
) COMMENT '对话消息表' CHARSET = utf8mb4 AUTO_INCREMENT = 10000001;
